...
(defn sue-matches-1 [sue facts]
  (every? true? (map (fn [[k v]] (or (nil? (k sue)) (= v (k sue)))) facts)))

(defn which-sue-matches [fn sues facts]
  (some #(if (fn (val %) facts) (key %)) sues))

(defn which-sue-matches-1 [sues facts]
  (which-sue-matches sue-matches-1 sues facts))
...
(defn sue-matches-2 [sue facts]
  (every? true? (map (fn [[k v]]
                       (or (nil? (k sue))
                           (cond
                             (k #{:cats :trees}) (> (k sue) v)
                             (k #{:pomeranians :goldfish}) (< (k sue) v)
                             :else (= (k sue) v))))
                     facts)))

(defn which-sue-matches-2 [sues facts]
  (which-sue-matches sue-matches-2 sues facts))
