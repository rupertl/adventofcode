...
fun findStartOfData(transmission: String, numDistinct: Int = 4): Int {
    assert(transmission.length >= numDistinct)
    for (index in numDistinct until transmission.length) {
        val extent = (index - numDistinct) until index
        if (transmission.slice(extent).toSet().size == numDistinct) {
            return index
        }
    }
    return 0
...
fun day06(input: String): String {
    val line = input.trim()

    val part1 = findStartOfData(line, 4)
    val part2 = findStartOfData(line, 14)
    return "$part1, $part2"
}
