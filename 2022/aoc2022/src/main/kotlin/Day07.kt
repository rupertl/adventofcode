...
//         immediately contained by the current directory: 123 abc
//         means that the current directory contains a file named abc
//         with size 123. dir xyz means that the current directory
...
class DeviceDirectory(val parent: DeviceDirectory?) {
    private val directories = mutableMapOf<String, DeviceDirectory>()
    private val files = mutableMapOf<String, Int>()

    fun addDirectory(name: String): DeviceDirectory {
        val newDir = DeviceDirectory(this)
    ...
    fun findDir(name: String): DeviceDirectory? = directories[name]

    fun dirSize(): Int = files.values.sum() + directories.values.sumOf { it.dirSize() }

    fun walkSizes(): List<Int> =
        directories.values.map { it.walkSizes() }.flatten() + dirSize()
}

fun parseFileActivity(lines: List<String>): DeviceDirectory {
    val root = DeviceDirectory(null)
    var dir = root
    for (line in lines) {
        if (line[0] == '$') {
            dir = parseFileCommand(dir, root, line)
        } else {
            parseDirectoryEntry(dir, line)
        }
    }
    return root
}

fun parseFileCommand(dir: DeviceDirectory, root: DeviceDirectory, line: String): DeviceDirectory {
    ...
    if (command == "cd") {
        ...
        if (dirName == "/") {
            ...
            if (parent == null) {
                throw RuntimeException("Can't cd .. from root dir")
            } else {
                return parent
            }
        } else {
            val child = dir.findDir(dirName)
            if (child == null) {
                throw RuntimeException("Can't cd to $dirName")
            } else {
                return child
            }
        }
    } else if (command == "ls") {
        // no action, will read dir listing next
    } else {
        throw RuntimeException("Invalid command $command")
    }
    return dir
}

fun parseDirectoryEntry(dir: DeviceDirectory, line: String) {
    val (spec, name) = line.split(' ')
    if (spec[0].isDigit()) {
        // file entry
        dir.addFile(fileName = name, fileSize = spec.toInt())
    } else if (spec == "dir") {
        // dir entry
        dir.addDirectory(name)
    } else {
        throw RuntimeException("Invalid dir entry $line")
    }
}
fun findSumAllDirectoriesBelowSize(
    root: DeviceDirectory,
    maxSize: Int = 100000
): Int {
    return root.walkSizes().filter { it <= maxSize }.sum()
}

fun findSmallestCandidateDirectory(
    root: DeviceDirectory,
    fileSystemSize: Int = 70000000,
    target: Int = 30000000
): Int {
    val currentFreeSpace = fileSystemSize - root.dirSize()
    val spaceToFind = target - currentFreeSpace
    return root.walkSizes().filter { it >= spaceToFind }.min()
}

fun day07(input: String): String {
    val lines = input.trim().lines()
    val root = parseFileActivity(lines)

    val part1 = findSumAllDirectoriesBelowSize(root)
    val part2 = findSmallestCandidateDirectory(root)
    return "$part1, $part2"
}
