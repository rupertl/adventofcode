...
// Unless, that is, you can design a replacement for the device's
// video system! It seems to be some kind of cathode-ray tube screen
// and simple CPU that are both driven by a precise clock circuit. The
// clock circuit ticks at a constant rate; each tick is called a
// cycle.
...
// (In this system, there is no such thing as "vertical position": if
// the sprite's horizontal position puts its pixels where the CRT is
...
class CPU(bufferSize: Int = 240, crtRows: Int = 6) {
    private var regX = 1
    private var clock = 1
    private val signal = Array(bufferSize + 1) { 0 }
    private var crt: MutableList<Char> = mutableListOf()
    private val pixelsPerRow = bufferSize / crtRows

    private fun inSprite(): Boolean {
        val rowPos = (clock - 1) % pixelsPerRow
        return (rowPos >= regX - 1) && (rowPos <= regX + 1)
    }

    private fun tick() {
        signal[clock] = clock * regX
        crt.add(if (inSprite()) '#' else '.')
        clock++
    }
    ...
    fun process(instruction: String) {
        ...
        if (words[0] == "noop") {
            ...
            throw RuntimeException("Invalid instruction $instruction")
        }
    }

    fun signalAt(time: Int) = signal[time]

    fun signalSum(offset: Int, increment: Int): Int {
        var sum = 0
        for (time in offset..clock step increment) {
            sum += signalAt(time)
        }
        return sum
    }
    ...
    fun renderCrt(): String = crt
        .chunked(pixelsPerRow)
...
fun day10(input: String): String {
    ...
    val part1 = cpu.signalSum(20, 40)
    val part2 = cpu.renderCrt().trim()
    return "$part1\n$part2"
}
