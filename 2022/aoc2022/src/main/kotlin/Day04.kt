...
class ElfCleaningPair(val elfA: IntRange, val elfB: IntRange) {

    val hasContainedSections =
        (elfA.first >= elfB.first && elfA.last <= elfB.last) ||
        (elfB.first >= elfA.first && elfB.last <= elfA.last)

    val hasOverlappedSections = ! ((elfA.first > elfB.last) ||
                                   (elfB.first > elfA.last))
}

fun makeElfCleaningPair(line: String): ElfCleaningPair {
    val pair = line.split(',')
    val a = pair[0].split('-')
    val b = pair[1].split('-')
    return ElfCleaningPair(
        IntRange(a[0].toInt(), a[1].toInt()),
        IntRange(b[0].toInt(), b[1].toInt())
    )
}

fun sumPairsWithContainedSections(lines: List<String>): Int =
    lines.map { makeElfCleaningPair(it) }.count { it.hasContainedSections }

fun sumPairsWithOverlappedSections(lines: List<String>): Int =
    lines.map { makeElfCleaningPair(it) }.count { it.hasOverlappedSections }

fun day04(input: String): String {
    val lines = input.trim().lines()
    val part1 = sumPairsWithContainedSections(lines)
    val part2 = sumPairsWithOverlappedSections(lines)
    return "$part1, $part2"
}
