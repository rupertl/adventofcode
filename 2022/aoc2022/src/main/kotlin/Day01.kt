...
fun countCalories(lines: List<String>) : PriorityQueue<Int> {
    val calories = PriorityQueue<Int>(Collections.reverseOrder())
    var currentElfCalories = 0
    for (line in lines) {
        val snack = line.toIntOrNull()
        if (snack == null) {
            // end of list for one elf
...
fun findElfWithMostCalories(calories: PriorityQueue<Int>): Int {
    return calories.peek()
}
...
fun findSumTop3Elves(calories: PriorityQueue<Int>) : Int {
    // NOTE: will mutate calories
    return calories.remove() + calories.remove() + calories.remove()
}

fun day01(input: String) : String {
    val calories = countCalories(input.lines())
    val mostCalories = findElfWithMostCalories(calories)
    val top3Calories = findSumTop3Elves(calories)
    return "$mostCalories, $top3Calories"
}
