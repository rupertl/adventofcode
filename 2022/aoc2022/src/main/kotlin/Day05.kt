...
// air conditioning, leather seats, an extra cup holder, and the
...
// so that the Elves know where they should stand to be ready to
// unload the final supplies. After the rearrangement procedure
// completes, what crate ends up on top of each stack?

// DATA SOURCE NOTE: I changed the input format to replace the stack
// diagram with a list of initial stack contents, eg for the above sample
// data the input would be "ZN,MCD,P".

class ShipStacks(initial: String, val model: Int = 9000) {
    private var stacks = initial.split(',').toTypedArray()

    fun getTops(): String = stacks.map { it.last() }.joinToString(separator = "")

    fun move(instruction: String) {
        var (quantity, from, to) = instruction
            .split(' ')
            .mapNotNull { it.toIntOrNull() }
        from--; to-- // as instructions are indexed from 1, but we index from 0
        val moving = stacks[from].takeLast(quantity)
        stacks[to] += when (model) {
            9000 -> moving.reversed()
            else -> moving
        }
        stacks[from] = stacks[from].dropLast(quantity)
    }
}
fun day05(input: String): String {
    val lines = input.trim().lines()
    val shipStacks9000 = ShipStacks(lines.first())
    val shipStacks9001 = ShipStacks(lines.first(), model = 9001)
    for (line in lines.drop(1)) {
        shipStacks9000.move(line)
        shipStacks9001.move(line)
    }

    val part1 = shipStacks9000.getTops()
    val part2 = shipStacks9001.getTops()
    return "$part1, $part2"
}
