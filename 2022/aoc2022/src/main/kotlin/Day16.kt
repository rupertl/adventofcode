...
// However, you need to release as much pressure as possible, so
...
// would release a total of 1707 unvisited pressure.

// With you and an elephant working together for 26 minutes, what is
// the most pressure you could release?

data class Node(val name: String, val rate: Int, val connections: List<String>)

class ValveGraph(
    lines: List<String>,
    minFlowToConsider: Int = 1,
) {
    private val startingPoint = "AA"
    private var maxTime: Int = 30
    val allNodes = parse(lines)
    val nodesToConsider = allNodes
        .filter { it.value.rate >= minFlowToConsider }
    val distances = calculateDistances()

    private fun parse(lines: List<String>): Map<String, Node> {
        val nodes = mutableMapOf<String, Node>()
        for (line in lines) {
            val m = Regex("Valve (\\w+) has flow rate=(\\d+); tunnels* leads* to valves* (.+)")
                .find(line)
            if (m == null) {
                throw RuntimeException("Can't parse $line")
            } else {
                val (name, rate, connections) = m.destructured
                nodes[name] = Node(name, rate.toInt(), connections.split(", "))
            }
        }
        return nodes
    }

    private fun calculateDistances(): Map<String, Map<String, Int>> {
        val allDists = mutableMapOf<String, Map<String, Int>>()
        for (node in nodesToConsider.keys + startingPoint) {
            val checked = mutableSetOf<String>()
            val dist = mutableMapOf<String, Int>()
            val work: Queue<String> = LinkedList()
            work.add(node)

            while (!work.isEmpty()) {
                val curr = work.remove()
                for (next in allNodes[curr]!!.connections) {
    ...
    fun findBestScore(timeLimit: Int = 30): Int {
        maxTime = timeLimit
        return findBestScoreForSubset(
            startingPoint,
            1,
            nodesToConsider.keys.toMutableSet(),
        )
    }

    private fun findBestScoreForSubset(at: String, time: Int, unvisited: Set<String>): Int {
        // Given the set of nodes in unvisited, find the subset of valves to
        // visit to maximise the score.
        val atScore = (maxTime - time + 1) * allNodes[at]!!.rate
        ...
        for (next in unvisited) {
            val newTime = time + distances[at]!![next]!! + 1
            if (newTime > maxTime) {
                // Not enough time to visit
                scores.add(0)
            } else {
                val remaining = unvisited.toMutableSet()
                remaining.remove(next)
                scores.add(findBestScoreForSubset(next, newTime, remaining))
            }
        }
        return atScore + scores.max()
    }

    fun findBestScoreWithElephant(timeLimit: Int = 26): Int {
        maxTime = timeLimit

        // Build allScores to be a map of unvisited items against score for the
        // visited items
        val allScores = mutableMapOf<Set<String>, Int>()
        findAllScores(
            startingPoint,
            1,
            nodesToConsider.keys.toMutableSet(),
        ...
        // Now find the best score for each set of unvisited items above
        // and add to the original score
        return allScores
            .map { it.value + findBestScoreForSubset(startingPoint, 1, it.key) }
            .max()
    }

    private fun findAllScores(
        at: String,
        time: Int,
        unvisited: Set<String>,
        currentScore: Int,
        allScores: MutableMap<Set<String>, Int>,
    ) {
        val newScore = currentScore + (maxTime - time + 1) * allNodes[at]!!.rate
        if (unvisited.isEmpty()) {
            // Nowhere to go
            allScores[unvisited] =
                max(allScores.getOrDefault(unvisited, 0), newScore)
        }

        // Add the score we'd get if we stopped now
        allScores[unvisited] =
            max(allScores.getOrDefault(unvisited, 0), newScore)
        ...
        for (next in unvisited) {
            ...
            if (newTime <= maxTime) {
                val remaining = unvisited.toMutableSet()
                remaining.remove(next)
                findAllScores(next, newTime, remaining, newScore, allScores)
            }
        }
    }
}

fun day16(input: String): String {
    val lines = input.trim().lines()
    val vg = ValveGraph(lines)

    val part1 = vg.findBestScore()
    val part2 = vg.findBestScoreWithElephant()
