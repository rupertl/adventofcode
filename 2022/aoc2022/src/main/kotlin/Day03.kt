...
//     hcsFMMfFFhFp. The only item type that appears in both
...
fun commonLuggageItem(luggage: String, numCompartments: Int = 2): Char {
    val compartments = Array(numCompartments) { mutableSetOf<Char>() }
    val compartmentSize = luggage.length / numCompartments
    for ((index, item) in luggage.withIndex()) {
        // Ignore spaces which act as padding
        if (item != ' ') {
            compartments[index / compartmentSize].add(item)
        }
    }
    for (index in 1 until numCompartments) {
        compartments[0].retainAll(compartments[index])
    }
    return compartments[0].first()
}

fun findSumOfPrioritiesOfCommonItems(luggage: List<String>): Int {
    return luggage.map { commonLuggageItem(it) }.sumOf { itemToPriority(it) }
}
...
fun findSumOfPrioritiesOfBadges(luggage: List<String>, groupSize: Int = 3): Int {
    // We need to pad each item of luggage until it has the same length.
    val maxLuggageSize = luggage.maxOfOrNull { it.length }
    var prioritySum = 0
    for (group in 0 until luggage.size / groupSize) {
        var groupLuggage = ""
        for (index in 0 until groupSize) {
            groupLuggage += luggage[(group * groupSize) + index]
                .padEnd(maxLuggageSize!!)
        }
        prioritySum += itemToPriority(commonLuggageItem(groupLuggage, groupSize))
    }
    return prioritySum
}
...
fun day03(input: String): String {
    ...
    val part1 = findSumOfPrioritiesOfCommonItems(lines)
    val part2 = findSumOfPrioritiesOfBadges(lines)
    return "$part1, $part2"
}
