...
class MonkeyTrouble(private val hasWorryDivider: Boolean = true) {
    private val worryDivisor = 3
    val monkeys = mutableListOf<Monkey>()
    private var modLcm = 1L

    fun parse(lines: List<String>) {
        for (group in lines.chunked(7)) {
            parseMonkey(group)
        }
        // Find the product of all tests to use later to reduce the worry size
        modLcm = monkeys.map { it.test }.reduce { acc, ele -> acc * ele }
    }

    private fun parseMonkey(group: List<String>) {
        // line 1: Starting items: 80, 99
        val items = group[1].split(": ")[1]
            .split(", ")
            .map { it.toLong() }
            .toMutableList()
        // line 2: Operation: new = old * 11
        val words = group[2].trim().split(" ")
        var operator = if (words[4] == "+") Operator.PLUS else Operator.TIMES
        var operand = 1L
        if (words[5].toIntOrNull() == null) {
            operator = Operator.SQUARE
        } else {
            operand = words[5].toLong()
        }
        // line 3: Test: divisible by 3
        val test = group[3].trim().split(" ")[3].toLong()
        // line 4: If true: throw to monkey 3
        val toTrue = group[4].trim().split(" ")[5].toInt()
        // line 5: If false: throw to monkey 5
        val toFalse = group[5].trim().split(" ")[5].toInt()
        monkeys.add(Monkey(items, operator, operand, test, toTrue, toFalse))
    }
    ...
    fun doRound() {
        for (monkey in monkeys) {
            ...
            for (item in monkey.items) {
                var newItem = when (monkey.operator) {
                    Operator.PLUS -> item + monkey.operand
                    Operator.TIMES -> item * monkey.operand
                    Operator.SQUARE -> item * item
                }
                if (hasWorryDivider) {
                    newItem /= worryDivisor
                } else {
                    newItem %= modLcm
                }
                if (newItem % monkey.test == 0L) {
                    monkeys[monkey.toTrue].items.add(newItem)
                } else {
                    monkeys[monkey.toFalse].items.add(newItem)
                }
            }
            monkey.items.clear()
        }
    ...
    fun doRounds(rounds: Int = 20) {
        for (index in 0 until rounds) {
            doRound()
        }
    }

    fun troubleScore(): Long {
...
fun day11(input: String): String {
    ...
    val mt1 = MonkeyTrouble()
    mt1.parse(lines)
    mt1.doRounds(20)
    val part1 = mt1.troubleScore()

    val mt2 = MonkeyTrouble(false)
    mt2.parse(lines)
    mt2.doRounds(10000)
    val part2 = mt2.troubleScore()
