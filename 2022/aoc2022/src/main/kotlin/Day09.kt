...
// able to model the positions of the knots on a two-dimensional grid.
// Then, by following a hypothetical series of motions (your puzzle
...
// seconds to choose how to arch your body to avoid being hit.
// Fortunately, your simulation can be extended to support longer
// ropes.
...
class Rope(numKnots: Int = 2, startingPosition: RopeKnot = RopeKnot(0, 0)) {
    val knots = Array(numKnots) { startingPosition.copy() }
    private val tailPositions = mutableSetOf(knots.last().toString())
    ...
    fun moveHead(instruction: String) {
        ...
        for (step in 1..steps) {
            ...
            tailPositions.add(knots.last().toString())
        }
    }

    private fun moveHeadDirection(direction: String) {
        when (direction) {
            "L" -> knots[0].x--
            "R" -> knots[0].x++
            "D" -> knots[0].y--
            "U" -> knots[0].y++
            else -> throw RuntimeException("Bad move $direction")
        }
    }

    private fun moveDist(diff: Int, diagonal: Boolean = false): Int = when (diff) {
        2 -> 1
        -2 -> -1
        else -> if (diagonal) diff else 0
    }

    private fun moveTail(head: RopeKnot, tail: RopeKnot) {
        var dx = moveDist(head.x - tail.x)
        var dy = moveDist(head.y - tail.y)
        if (dx != 0 && head.y != tail.y) {
            dy = moveDist(head.y - tail.y, true)
        } else if (dy != 0 && head.x != tail.x) {
            dx = moveDist(head.x - tail.x, true)
        }
        tail.x += dx
        tail.y += dy
    }

    fun numTailPositions() = tailPositions.size
}

fun day09(input: String): String {
    ...
    val part1 = r2.numTailPositions()
    val part2 = r10.numTailPositions()
    return "$part1, $part2"
}
